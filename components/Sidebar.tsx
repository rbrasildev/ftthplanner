                ...
                <div className={`p-4 border-t border-slate-100 dark:border-slate-900 ${isCollapsed ? 'items-center' : ''} flex flex-col gap-3 flex-shrink-0`}>
                    ...
                    {!isCollapsed && isHydrated && userRole !== 'support' && (
                        <div className="px-2 mb-1">
                                <Button
                                    variant={(userPlan === 'Plano Grátis' || userPlanType?.toUpperCase() === 'TRIAL') ? 'primary' : 'outline'}
...
const NavButton: React.FC<NavButtonProps> = ({ icon, label, onClick, isCollapsed, variant = 'slate', progress, isActive, hasArrow, isExpanded }) => {
    ...
    return (
        ...
        >
            <div className={`flex-shrink-0 transition-transform duration-200 group-hover:scale-110 ${isEmerald ? 'text-white' : 'text-slate-400 group-hover:text-slate-900 dark:group-hover:text-slate-100'}`}>
                {icon}
            </div>
