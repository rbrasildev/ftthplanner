...
export const MarkersCanvasLayer: React.FC<Props> = ({
    ...
    useEffect(() => {
        ...
        // Expõe o hit-test no map pra outros layers que interceptam clique em
        // capture phase (D3CablesLayer) poderem ceder quando o ponto cai sobre
        // um marker do canvas. Sem isso, o cabo "ganha" o clique porque a
        // ordem de registro do listener favorece quem montou primeiro.
        // O `extraPad` permite ao cabo usar uma zona de exclusão maior que a
        // zona real de click — assim a "área da CTO" fica visualmente generosa
        // sem interferir no hit-test preciso do próprio marker.
        (map as any)._canvasMarkerHitTest = hitTest;
        ...
        return () => {
            ...
            container.style.cursor = '';
            if ((map as any)._canvasMarkerHitTest === hitTest) {
                delete (map as any)._canvasMarkerHitTest;
            }
        };
    }, [map, visible]);
