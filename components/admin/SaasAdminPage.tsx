...
const CompanyInvoicesSection: React.FC<{ companyId: string, company: Company, financial?: { overdueCount: number, overdueTotal: number, paidCount: number, paidTotal: number, lastPayment: string | null } }> = ({ companyId, company, financial }) => {
    ...
    const handleRecalculate = async () => {
        if (!window.confirm('Recalcular financeiro? Vai ajustar a data de expiração e o status com base nas faturas pagas.')) return;
        setRecalculating(true);
        try {
            const result = await saasService.recalculateCompanyFinancials(companyId);
            alert(`Status: ${result.previousStatus} → ${result.newStatus}\nExpiração: ${result.previousExpiry ? new Date(result.previousExpiry).toLocaleDateString('pt-BR') : '—'} → ${result.newExpiry ? new Date(result.newExpiry).toLocaleDateString('pt-BR') : '—'}\n\nRecarregue a página pra ver os dados atualizados.`);
        } catch (err) {
            console.error('Failed to recalculate', err);
            alert('Erro ao recalcular financeiro.');
        } finally {
                    ...
                    <div className="flex items-center gap-2 mb-1 shrink-0">
                        {activeTab === 'summary' && (
                            <button
                                ...
                                title="Recalcular expiração e status baseado nas faturas pagas. Útil pra consertar empresas que ficaram em estado inconsistente (suspensa mesmo após pagar tudo)."
                                className="px-2.5 py-1 text-[10px] font-bold text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-700/40 hover:bg-slate-200 dark:hover:bg-slate-700/70 rounded transition-colors disabled:opacity-50"
                            >
                        ...
                        <button
                            ...
                            className="px-2.5 py-1 text-[10px] font-bold text-emerald-700 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-900/20 hover:bg-emerald-100 dark:hover:bg-emerald-900/40 rounded transition-colors disabled:opacity-50"
                        >
                            + Gerar adiantadas
                        </button>
                    </div>
            ...
            {activeTab !== 'summary' && (() => {
                ...
                const renderInvoice = (inv: any) => {
                    ...
                    const daysLate = isOverdue && dueDate
                        ...
                        : 0;
                    return (
                        <div key={inv.id} className={`p-3 rounded-lg border text-xs ${isOverdue ? 'bg-red-50 dark:bg-red-950/20 border-red-200 dark:border-red-800' : isCancelled ? 'bg-slate-50 dark:bg-slate-800/30 border-slate-200 dark:border-slate-700/30 opacity-75' : 'bg-white dark:bg-[#1a1d23] border-slate-100 dark:border-slate-700/30'}`}>
    ...
    return (
        <div className="flex h-screen bg-[#f9fafb] dark:bg-[#151820] font-sans text-slate-900 dark:text-slate-100 overflow-hidden">
            ...
            <aside className={`bg-white dark:bg-[#1a1d23] border-r border-slate-200 dark:border-slate-700/30 flex flex-col z-20 shadow-xl transition-all duration-300 relative ${isCollapsed ? 'w-20' : 'w-64'}`}>
                ...
                <div className={`p-6 flex items-center gap-3 border-b border-slate-100 dark:border-slate-700/50 overflow-hidden ${isCollapsed ? 'justify-center px-4' : ''}`}>
                                                        ...
                                                        return (
                                                            <div className="inline-flex flex-col items-center gap-0.5">
                                                                <input
                                                                    ...
                                                                    className={`bg-white dark:bg-[#1a1d23] border rounded-lg text-xs font-bold py-1.5 px-2.5 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-400 cursor-pointer shadow-sm hover:border-emerald-300 dark:hover:border-emerald-600 transition-colors ${expired ? 'border-red-300 text-red-600 dark:border-red-700' : 'border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300'}`}
                                                ...
                                                <tr key={user.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                                    ...
                                                    <td className="px-6 py-4">
                                                        <select
                                                            ...
                                                            className="bg-white dark:bg-[#1a1d23] border border-slate-200 dark:border-slate-700 text-xs rounded py-1 px-2"
                                        ...
                                        <div>
                                            ...
                                            <div className="space-y-2 max-h-60 overflow-y-auto pr-2">
                                                {selectedCompany.users?.map(u => (
                                                    <div key={u.id} className="flex items-center justify-between p-3 bg-white dark:bg-[#1a1d23] border border-slate-100 dark:border-slate-700/30 rounded-lg">
                                        ...
                                        <div>
                                            ...
                                            <div className="space-y-2 max-h-60 overflow-y-auto pr-2">
                                                {selectedCompany.projects?.map(p => (
                                                    <div key={p.id} className="flex items-center justify-between p-3 bg-white dark:bg-[#1a1d23] border border-slate-100 dark:border-slate-700/30 rounded-lg hover:border-emerald-300 transition-colors">
                                                        <div className="flex items-center gap-3 min-w-0">
